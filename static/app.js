class Chatbox {
  constructor() {
    ...
    this.state = false;
    this.message = [{ name: "Sam", message: "Hello, I'm AI Version of Bhumika (Currently in Beta)! Ask me anything!"}];
  }
  ...
  updateChatText(chatBox) {
    ...
    this.message
      ...
      .forEach(function (item) {
        ...
        if (item.name === "Sam") {
          html +=
            '<div style="display: flex; align-items: end; gap:0.5rem;"><img style="width: 40px; height: 40px; border-radius: 50%;" src="../static/images/me3.jpeg" alt="image"> <div style="font-size: 0.8rem"  class="messages__item messages__item--visitor">' +
            item.message +
            "</div></div>";
        } else {
          html +=
            '<div style="font-size: 0.8rem" class="messages__item messages__item--operator">' +
            item.message +
            "</div>";
        }
      });
